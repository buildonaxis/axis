...
describe("parseIdentity", () => {
    ...
    it("parses expiration date", () => {
        const result = parseIdentity("010003123456789017261231");
        expect(result.expiration).toBe("2026-12-31");
    });
    it("parses GTIN, expiration, lot, and serial", () => {
        const result = parseIdentity("01000312345678901726123110LOT12321ABC123");
        expect(result.gtin).toBe("00031234567890");
        expect(result.expiration).toBe("2026-12-31");
        expect(result.lot).toBe("LOT123");
        expect(result.serial).toBe("ABC123");
