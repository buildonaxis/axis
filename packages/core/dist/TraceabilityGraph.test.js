...
describe("TraceabilityGraph", () => {
    ...
    it("stores aggregations", () => {
        const graph = new TraceabilityGraph();
        const parent = SerializedItem.fromBarcode("010031234567890121ABC123");
        const child = SerializedItem.fromBarcode("010031234567890121DEF456");
...
it("finds items by epc", () => {
    ...
    const parent = SerializedItem.fromBarcode("010031234567890121ABC123");
    const child = SerializedItem.fromBarcode("010031234567890121DEF456");
    graph.addAggregation(new Aggregation(parent, [child]));
    const result = graph.findItem(child.toEpcUri());
